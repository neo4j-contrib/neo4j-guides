...
****
[NOTE]
All scenarios in this guide are hypothetical business cases and are not based on real events at Neo4j or any other specific company.
****
...
The marketing department might want to see how well a marketing campaigns did to generate sales.
...
In our query above, we first set the campaign publish timestamp as a variable and then pass that to the next line, where we also use that `startDate` to calculate our end date (using the procedure).
...
//focus on how to use APOC to convert dates to native Neo4j datetime types
...
1. the date string that needs converted
2. what the format is of the date string (1st parameter)
...
The `orderDate` is converted using the procedure and specifying the string format it is currently in, and the `iso_date_time` format (2019-01-01T00:00:00) we want to have as the result.
